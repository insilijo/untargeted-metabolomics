...
fsf=fn.source_file.to_numpy()
o=np.argsort(fmz); fmz,frt,fin,fsf=fmz[o],frt[o],fin[o],fsf[o]
...
# ---- diagnostic: multi-MAF isobaric groups (co-eluting isomers Metabolon resolved by RI, not MS2) ----
mafpc=[sum(1 for k in c if inmaf[k]) for c in cl]
print(f"  naive clusters {len(cl)}; clusters holding >=2 MAF (RT+mz fuses RI-distinct isomers): {sum(1 for x in mafpc if x>=2)}",flush=True)

# ---- final predicted RT per candidate ----
predRT=np.full(n,np.nan); prf=mdl.predict(DX)
for ii,k in enumerate(withd): predRT[k]=prf[ii]

# ---- isobaric m/z groups over the candidate library ----
...
# ---- reproducible feature peaks at an m/z (distinct-RT peaks, >=MINREP injections) ----
def group_peaks(mz):
    t=mz*MZ_PPM*1e-6; lo=np.searchsorted(fmz,mz-t); hi=np.searchsorted(fmz,mz+t)
    if hi<=lo: return []
    rts=frt[lo:hi]; its=fin[lo:hi]; sfs=fsf[lo:hi]
    rtb=defaultdict(list)
    for r,it,sf in zip(rts,its,sfs): rtb[int(r//(2*TIGHT))].append((r,it,sf))
    dist=[(float(np.median([r for r,_,_ in v])), max(i for _,i,_ in v))
          for v in rtb.values() if len(set(sf for _,_,sf in v))>=MINREP]
    return dist
...
for g in groups:
    ...
    gg=[k for k in g if not np.isnan(predRT[k])]
    if not gg: continue
